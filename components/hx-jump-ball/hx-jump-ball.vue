  ...
  <view class="hx-jump-ball">
    <view class="ballBox" :animation="ballBoxAnimationData">
      <view
        ...
        :style="{
          width: ballWidth * 2 + 'upx',
          height: ballHeight * 2 + 'upx',
...
<script>
  // Todo 多渲染一些 view 连续点击时可以同时显示多个小球
export default {
  name: 'hx-jump-ball',
  data() {
    return {
  ...
  props: {
    ...
    bezier: {
      type: String,
      default: 'cubic-bezier(.7,-0.3,.94,.71)'
    }
  },
  ...
  methods: {
    //获取元素坐标
    getElementCoordinate() {
      let that = this
      // const query = uni.createSelectorQuery();
      // let nodesRef = query.select(startElement);

      const SLeft = this.event.x
      const STop = this.event.y

      let nodesRef2 = uni
        .createSelectorQuery()
        .select('.popupHistoryTakingBall')
      nodesRef2
        .fields(
          {
            id: true,
          ...
          res2 => {
            ...
            //计算出元素的中心坐标
            let ELeft =
              res2.left + ((res2.width + that.ballWidth) / 2 - that.ballWidth)
            let ETop =
              res2.bottom -
              ((res2.height - that.ballHeight) / 2 + that.ballHeight)
            //初始化位置
      ...
      // 	  let nodesRef2 = uni.createSelectorQuery().select('.popupHistoryTakingBall');
      // 	  nodesRef2.fields({
      // 	        id: true,
      // 	        rect: true,
      // 	  	  size: true
    ...
    startAnimation(SLeft, STop, ELeft, ETop) {
      ...
      setTimeout(function() {
        //根坐标
        that.ballBoxAnimation
          .opacity(1)
          .translate3d(ELeft, ETop, 0)
          .step({ duration: that.speed })
        that.ballBoxAnimationData = that.ballBoxAnimation.export()

        //相对根的坐标
        that.ballOuterAnimation
          .opacity(1)
          .translate3d(0, 0, 0)
          .step({ duration: that.speed })
    ...
    setStart(SLeft, STop, ELeft, ETop) {
      this.ballBoxAnimation
        .translate3d(ELeft, STop, 0)
        .opacity(1)
        .step({ duration: 0 })
      this.ballBoxAnimationData = this.ballBoxAnimation.export()

      this.ballOuterAnimation
        .translate3d(SLeft - ELeft, 0, 0)
        .opacity(1)
        .step({ duration: 0 })
      this.ballOuterAnimationData = this.ballOuterAnimation.export()
    },
